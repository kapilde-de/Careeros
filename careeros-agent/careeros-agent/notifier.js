// ════════════════════════════════════════════════════════════════════════
// Notifier — email reports via Resend API (resend.com)
// Setup: sign up at resend.com → API Keys → Create Key (free, 3k/month)
...
export class Notifier {
  ...
  async sendApplicationReport(applications) {
    ...
    const filled = applications.filter(a => a.status === "filled_pending_review" || a.status === "manual_required");
    const failed = applications.filter(a => a.status === "failed");

    const subject = `🤖 CareerOS — ${submitted.length} applications submitted`;
...
<body>
  ...
  <div class="footer">
    CareerOS Agent · <a href="http://localhost:3939">View Dashboard</a>
  </div>
</body>
</html>`;
...
<html><body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px">
  ...
  async send(subject, html) {
    if (!this.resend) {
      console.log(`\n📊 [Email skipped — no RESEND_API_KEY] ${subject}`);
      return;
    }
    try {
      const from = this.config.resendFromEmail || "CareerOS Agent <onboarding@resend.dev>";
      const to = this.config.emailTo;
      await this.resend.emails.send({ from, to, subject, html });
      console.log(`📧 Report sent to ${to}`);
    } catch (err) {
      console.error("❌ Email failed:", err.message);
      console.log("   → Check RESEND_API_KEY in your .env");
    }
  }
  ...
  async sendError(err) {
    ...
    if (this.resend) {
      await this.send(
        "❌ CareerOS Agent Error",
        `<p style="font-family:Arial">Agent error: <strong>${err.message}</strong></p><p>Check your terminal for details.</p>`
      ).catch(() => {});
    }
  }
}
