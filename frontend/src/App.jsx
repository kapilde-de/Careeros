...
const RESUME_FORMATS = [
  { id:"apex",       name:"Apex",        desc:"Dark header, commanding presence",    icon:"🏔️", accentColor:"#1e3a5f" },
  { id:"horizon",    name:"Horizon",     desc:"Two-column sidebar layout",           icon:"🌅", accentColor:"#0d9488" },
  { id:"pinnacle",   name:"Pinnacle",    desc:"Elegant serif, executive style",      icon:"✦",  accentColor:"#1e3a8a" },
  { id:"slate",      name:"Slate",       desc:"Bold monochrome, strong typography",  icon:"⬛", accentColor:"#111827" },
  { id:"prism",      name:"Prism",       desc:"Vibrant gradient, modern design",     icon:"💎", accentColor:"#7c3aed" },
  { id:"foundation", name:"Foundation",  desc:"ATS-safe, clean, max readability",    icon:"🤖", accentColor:"#0f766e" },
];
...
function generateResumeHTML(resume, format) {
  ...
  const expHTML = (accentColor, bulletChar="•", titleColor="#111", companyColor="#555") =>
    r.experience.map(e=>`
      <div style="margin-bottom:18px">
        ...
        <div style="font-size:12px;color:${companyColor};margin-bottom:6px;font-weight:500">${e.company||""}</div>
        ${(e.bullets||[]).map(b=>`<div style="font-size:12px;color:#374151;padding-left:16px;position:relative;margin-bottom:4px;line-height:1.5"><span style="position:absolute;left:0;color:${accentColor};font-weight:700">${bulletChar}</span>${b}</div>`).join("")}
      </div>`).join("");
  ...
  const certHTML = (color="#374151",dot="▸") => r.certifications.length ? `<div style="margin-top:6px">${r.certifications.map(c=>`<div style="font-size:12px;color:${color};margin-bottom:4px"><span style="color:#888">${dot}</span> ${c}</div>`).join("")}</div>` : "";
  ...
  // ── APEX: dark navy header, white text, clean single column ─────────────────
  if (format==="apex") return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
    *{margin:0;padding:0;box-sizing:border-box}
    body{font-family:'Calibri',Arial,sans-serif;background:#fff;color:#1a1a2e;font-size:13px}
    ...
    .exp-title{font-size:13.5px;font-weight:700;color:#1e3a5f}
    .exp-meta{font-size:11.5px;color:#64748b;font-style:italic;margin-bottom:5px}
    ...
    <div style="background:linear-gradient(135deg,#1e3a5f 0%,#1e4d7b 100%);padding:30px 38px 24px">
      ...
      <div style="font-size:12px;color:rgba(255,255,255,0.72);letter-spacing:0.4px">${r.contact}</div>
    </div>
    <div style="padding:20px 38px 32px">
      <div class="sec">Professional Summary</div>
      <p style="font-size:12.5px;line-height:1.75;color:#374151">${r.summary}</p>
      <div class="sec">Core Skills</div>
      <div>${skillTags("#eef2ff","#1e3a5f","1px solid #c7d2fe")}</div>
      <div class="sec">Experience</div>
      ${r.experience.map(e=>`
        <div style="margin-bottom:20px">
          <div style="display:flex;justify-content:space-between;align-items:baseline;flex-wrap:wrap;gap:4px;margin-bottom:3px">
            <span class="exp-title">${e.title||""}</span>
            <span style="font-size:11px;color:#94a3b8;font-style:italic">${e.period||""}</span>
          </div>
          <div class="exp-meta">${e.company||""}</div>
          ${(e.bullets||[]).map(b=>`<div class="bullet">${b}</div>`).join("")}
        </div>`).join("")}
      <div class="sec">Education</div>
      ${r.education.map(e=>`<div style="font-size:12px;color:#374151;margin-bottom:5px;padding-left:4px;border-left:3px solid #c7d2fe">  ${e}</div>`).join("")}
  ...
  // ── HORIZON: two-column sidebar ─────────────────────────────────────────────
  if (format==="horizon") return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Calibri',sans-serif;display:flex;min-height:100vh;background:#fff}.sidebar{width:32%;background:#0d4f4a;color:#fff;padding:28px 20px;flex-shrink:0}.main{flex:1;padding:28px 28px}.sh{font-size:9px;font-weight:800;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,0.6);margin:18px 0 6px;padding-bottom:5px;border-bottom:1px solid rgba(255,255,255,0.2)}.mh{font-size:10px;font-weight:800;letter-spacing:2px;text-transform:uppercase;color:#0d9488;margin:18px 0 6px;padding-bottom:5px;border-bottom:2px solid #e5f9f7}</style></head><body>
    <div class="sidebar">
      <div style="font-size:22px;font-weight:800;line-height:1.2;margin-bottom:6px;color:#fff">${r.name}</div>
      <div style="font-size:11px;color:rgba(255,255,255,0.65);line-height:1.7;margin-bottom:4px">${r.contact.replace(/•/g,"<br>")}</div>
      <div class="sh">Skills</div>
      ${r.skills.map(s=>`<div style="font-size:11px;color:rgba(255,255,255,0.85);padding:3px 0;border-bottom:1px solid rgba(255,255,255,0.1)">${s}</div>`).join("")}
    ...
    <div class="main">
      ...
      ${expHTML("#0d9488","●","#111827","#6b7280")}
      <div class="mh">Education</div>
      ${eduHTML("#374151")}
    </div>
  </body></html>`;

  // ── PINNACLE: elegant serif, centered name, thin ornamental lines ────────────
  if (format==="pinnacle") return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Georgia',serif;background:#fff;color:#1a1a2e;max-width:760px;margin:0 auto;padding:36px 44px}.divider{display:flex;align-items:center;gap:10px;margin:18px 0 10px}.divider::before,.divider::after{content:'';flex:1;height:1px;background:#1e3a8a30}.dh{font-size:10px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:#1e3a8a;white-space:nowrap}</style></head><body>
    <div style="text-align:center;margin-bottom:6px">
      <div style="font-size:30px;font-weight:700;color:#1e3a8a;letter-spacing:-0.5px">${r.name}</div>
      <div style="font-size:12px;color:#64748b;margin-top:5px;letter-spacing:0.5px">${r.contact}</div>
      <div style="display:flex;justify-content:center;gap:6px;margin-top:8px;align-items:center">
        <div style="height:1px;width:60px;background:#1e3a8a"></div>
        <div style="width:6px;height:6px;background:#1e3a8a;transform:rotate(45deg)"></div>
        <div style="height:1px;width:60px;background:#1e3a8a"></div>
      </div>
    </div>
    <div class="divider"><span class="dh">Professional Summary</span></div>
    <p style="font-size:12.5px;line-height:1.8;color:#374151;font-style:italic">${r.summary}</p>
    <div class="divider"><span class="dh">Core Skills</span></div>
    <div>${skillTags("transparent","#1e3a8a","1px solid #1e3a8a50")}</div>
    <div class="divider"><span class="dh">Experience</span></div>
    ${expHTML("#1e3a8a","◆","#1e3a8a","#64748b")}
    <div class="divider"><span class="dh">Education</span></div>
    ${eduHTML()}
    ${r.certifications.length?`<div class="divider"><span class="dh">Certifications</span></div>${certHTML("#374151","◆")}`:""}
  </body></html>`;

  // ── SLATE: bold monochrome, black section bars ───────────────────────────────
  if (format==="slate") return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Arial',sans-serif;background:#fff;color:#111;max-width:760px;margin:0 auto;padding:36px 44px}.sbar{background:#111;color:#fff;font-size:9px;font-weight:800;letter-spacing:3px;text-transform:uppercase;padding:5px 12px;display:inline-block;margin:20px 0 10px}</style></head><body>
    <div style="border-left:6px solid #111;padding-left:16px;margin-bottom:20px">
      <div style="font-size:32px;font-weight:900;color:#111;letter-spacing:-1px;text-transform:uppercase;line-height:1">${r.name}</div>
      <div style="font-size:12px;color:#555;margin-top:6px;letter-spacing:0.5px">${r.contact}</div>
    </div>
    <div class="sbar">Summary</div>
    <p style="font-size:12.5px;line-height:1.75;color:#222">${r.summary}</p>
    <div class="sbar">Skills</div>
    <div>${skillTags("#f3f4f6","#111","1px solid #d1d5db")}</div>
    <div class="sbar">Experience</div>
    ${expHTML("#111","■","#111","#444")}
    <div class="sbar">Education</div>
    ${eduHTML("#222")}
    ${r.certifications.length?`<div class="sbar">Certifications</div>${certHTML("#222","■")}`:""}
  </body></html>`;

  // ── PRISM: gradient header, purple accents, modern ───────────────────────────
  if (format==="prism") return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Helvetica Neue',Helvetica,sans-serif;background:#fff;color:#111}.ph{font-size:10px;font-weight:800;letter-spacing:2px;text-transform:uppercase;color:#7c3aed;margin:18px 0 8px;display:flex;align-items:center;gap:8px}.ph::after{content:'';flex:1;height:2px;background:linear-gradient(90deg,#7c3aed22,transparent)}</style></head><body>
    ...
    <div style="padding:20px 36px">
      <div class="ph">Profile</div>
      <p style="font-size:12.5px;line-height:1.75;color:#374151">${r.summary}</p>
      <div class="ph">Skills</div>
      <div>${skillTags("#f5f3ff","#6d28d9","1px solid #ddd6fe")}</div>
      <div class="ph">Experience</div>
      ${expHTML("#7c3aed","◉","#1e1b4b","#6b7280")}
  ...
  // ── FOUNDATION: ATS-safe plain text ─────────────────────────────────────────
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Arial',sans-serif;background:#fff;color:#111;max-width:760px;margin:0 auto;padding:36px 44px;font-size:12px;line-height:1.6}hr{border:none;border-top:1px solid #333;margin:10px 0}h2{font-size:12px;font-weight:700;text-transform:uppercase;margin:14px 0 4px;letter-spacing:0.5px}</style></head><body>
    <div style="font-size:22px;font-weight:700;margin-bottom:3px">${r.name}</div>
    <div style="color:#444;margin-bottom:6px">${r.contact}</div>
    <hr>
    <h2>Summary</h2>
    <p style="margin-bottom:4px">${r.summary}</p>
    <h2>Skills</h2>
    <p style="margin-bottom:4px">${r.skills.join(" | ")}</p>
    <h2>Experience</h2>
    ${r.experience.map(e=>`<div style="margin-bottom:12px"><div style="font-weight:700">${e.title} — ${e.company} (${e.period})</div>${(e.bullets||[]).map(b=>`<div style="padding-left:14px">• ${b}</div>`).join("")}</div>`).join("")}
    <h2>Education</h2>
    ${r.education.map(e=>`<div>${e}</div>`).join("")}
      ...
      <div style={{display:"flex",gap:7,marginBottom:12,overflowX:"auto",paddingBottom:4}}>
        {RESUME_FORMATS.map(f=>(
          <div key={f.id} onClick={()=>onSelect(f.id)}
            style={{border:`2px solid ${selected===f.id?f.accentColor:"#e2e8f0"}`,borderRadius:8,padding:"8px 10px",cursor:"pointer",background:selected===f.id?f.accentColor+"15":"#f8fafc",transition:"all 0.15s",textAlign:"center",flexShrink:0,minWidth:80}}>
            <div style={{fontSize:18,marginBottom:3}}>{f.icon}</div>
            <div style={{fontSize:10,fontWeight:700,color:selected===f.id?f.accentColor:"#64748b"}}>{f.name}</div>
          </div>
        ))}
      </div>
    ...
    <motion.div key="agent" variants={fadeUp} initial="initial" animate="animate" exit="exit" style={{maxWidth:920,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:12}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          {agentActive
            ? <button onClick={deactivateAgent} style={ghost({fontSize:12,padding:"8px 14px",color:"#dc2626",borderColor:"rgba(220,38,38,0.25)"})}>⏸ Pause Agent</button>
            : user&&<button onClick={activateAgent} disabled={activating} style={{...ghost({fontSize:12,padding:"8px 14px",color:"#059669",borderColor:"rgba(5,150,105,0.3)"}),opacity:activating?0.7:1}}>{activating?"Activating…":"▶ Activate Daily Scan"}</button>}
          {agentStatus!=="running"&&<button onClick={runScan} style={{...ghost({fontSize:12,padding:"8px 14px",color:"#7c3aed",borderColor:"rgba(124,58,237,0.3)"})}}>⚡ Scan Now</button>}
                ...
                <div className="fmt-scroll" style={{display:"flex",gap:7,overflowX:"auto",paddingBottom:2}}>
                  {RESUME_FORMATS.map(f=>(
                    <div key={f.id} onClick={()=>setFmt(f.id)}
                      style={{border:`2px solid ${fmt===f.id?f.accentColor:"#e2e8f0"}`,borderRadius:8,padding:"8px 12px",cursor:"pointer",background:fmt===f.id?f.accentColor+"15":"#f8fafc",transition:"all 0.15s",textAlign:"center",flexShrink:0,minWidth:80}}>
                      <div style={{fontSize:16,marginBottom:3}}>{f.icon}</div>
                      <div style={{fontSize:10,fontWeight:700,color:fmt===f.id?f.accentColor:"#64748b"}}>{f.name}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {RESUME_FORMATS.map((f,i)=>{
                ...
                return (
                  <motion.div key={f.id} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:i*0.08}}
                    ...
                    onMouseEnter={e=>{e.currentTarget.style.borderColor=f.accentColor+"80";e.currentTarget.style.boxShadow=`0 8px 32px ${f.accentColor}20`;e.currentTarget.style.transform="translateY(-2px)";}}
                    ...
                    <div style={{height:5,background:f.accentColor}}/>
                    ...
                    <div style={{padding:"14px 16px",borderTop:"1px solid #f1f5f9",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
                      ...
                      <div style={{display:"flex",flexDirection:"column",gap:4,flexShrink:0}}>
                        <button onClick={()=>setTab("builder")} style={{background:f.accentColor,color:"#fff",border:"none",borderRadius:7,padding:"6px 14px",fontSize:11,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",letterSpacing:"0.01em"}}>Use →</button>
                        {result?.resume&&<button onClick={()=>downloadResume(result.resume,f.id)} style={{background:"transparent",color:f.accentColor,border:`1.5px solid ${f.accentColor}50`,borderRadius:7,padding:"5px 14px",fontSize:11,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap"}}>⬇ Download</button>}
                      </div>
                    </div>
                  </motion.div>
                );
